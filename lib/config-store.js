 ...
 * Persists: apps, hotkeys (mute `hotkey` + pause `pauseHotkey`), mute
 * state, user preferences, window bounds.
    ...
    try {
      if (fs.existsSync(this.filePath)) {
        ...
        this.state = {
          ...
          // Migrate legacy entries (single `hotkey`) to the dual-hotkey
          // schema: `hotkey` = mute toggle, `pauseHotkey` = pause/resume.
          apps: apps.map((a) => ({ hotkey: '', pauseHotkey: '', ...a })),
        };
      }
    } catch {
      this.state = clone(DEFAULT_STATE);
