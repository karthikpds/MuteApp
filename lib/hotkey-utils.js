 ...
 * a conflict matches either slot. When `exceptField` ('hotkey' or
 * 'pauseHotkey') is given, only that one slot of `exceptId` is skipped, so
 * assigning the same accelerator to an app's own other slot still counts
 * as a conflict.
...
function findConflict(apps, acc, exceptId = null, exceptField = null) {
  ...
  for (const a of apps || []) {
    if (a.hotkey && a.hotkey.toLowerCase() === want) {
      if (exceptId && a.id === exceptId && (exceptField === null || exceptField === 'hotkey')) continue;
      return a;
    }
    if (a.pauseHotkey && a.pauseHotkey.toLowerCase() === want) {
      if (exceptId && a.id === exceptId && (exceptField === null || exceptField === 'pauseHotkey')) continue;
      return a;
    }
  }
  return null;
